...
const logout = async (req:Request,res:Response):Promise<void>=>{
    try {
        res.status(200).clearCookie("token").json({success:true,message:"Logout successfully"}as IResponse)
    } catch (error:any) {
        console.log(error.message);
        
        res.status(500).json({success:false,message:"logout internal error",error:error.message}as IResponse)
    }
}
